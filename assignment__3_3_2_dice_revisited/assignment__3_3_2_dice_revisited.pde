...
void mousePressed() {
  //if (mouseX>dicePositionX && mouseX < dicePositionX + diceSize &&
  //  mouseY > dicePositionY && mouseY < dicePositionY + diceSize) {
  //  random = (int)random(1, 7); 
  //  println(random);
  //}
  boolean mouseOutside = mouseY < dicePositionY || mouseY > dicePositionY + diceSize 
                      || mouseX < dicePositionX || mouseX > dicePositionX + diceSize;
  if (!mouseOutside) {
    random = (int)random(1, 7); 
    println(random);
  }
